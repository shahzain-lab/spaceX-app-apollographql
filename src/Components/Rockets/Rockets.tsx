        ...
        <div className="flex flex-wrap justify-around h-auto w-full">
            {
                data?.rockets?.map(rocket => (
                    <div className="flex flex-col justify-center bg-gray-900 md:w-1/3 w-6/12 md:h-2/3 h-1/2 p-3 rounded-lg m-2 " >
                        {
                            rocket?.flickr_images?.slice(0, 1).map((image, i) => (
                                <div className="w-full h-72 pb-2 bg-purple-900 bg-opacity-100 bg-cover bg-no-repeat" key={i} style={{ backgroundImage: 'linear-gradient(rgba(135, 80, 156, 0.9), rgba(135, 80, 156, 0.9)), url(' + image + ')' }}>
